...
- Prerequisites:
  ...
  - The command step uses `commands[].name`. Provide command arguments with `--var command_extra_params_json='<json-object>'` and, when `with_file=true`, `--var command_file_id=<file-id>`.
- Shell note:
...
- Stop/decision gates:
  ...
  - Stop if `organization.models.getModel` has no matching `commands[].name` for the target device model, or if its command arguments cannot be validated from that model metadata.
...
- Prerequisites:
  ...
  - The built-in flow sends the selected `commands[].name` under request field `command`; it does not select by `friendly_name`, and request field `name` is invalid.
  - If the selected command has `custom_fields`, provide `--var command_extra_params_json='<json-object>'`; labels and label arrays are mapped through static `options` embedded in the model metadata to the exact scalar or array values. Malformed or unresolved choices stop before sending. If it has `with_file=true`, provide `--var command_file_id=<file-id>`.
...
- Stop/decision gates:
  ...
  - Stop if command or option metadata is malformed, command/custom-field definitions are duplicated, or if `command_extra_params_json` is not an object, contains undeclared keys, omits a required field, does not match a declared value type or scalar/array shape, cannot be mapped unambiguously from embedded static options, or relies on unresolved path-backed dynamic choices.
- Failure handling:
  ...
  - If the send is interrupted before its result is recorded, resume stops instead of sending the command again.
