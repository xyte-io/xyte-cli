...
3. For Edge, run model discovery before any claim write: `xyte-cli edge models list --tenant <tenant-id> --page 1 --per-page 100`, then `xyte-cli edge models describe --tenant <tenant-id> --model-id <model-id>`. Use the returned model id and `parameters[].name` labels when filling `device_model_id` and `custom_parameters`.
