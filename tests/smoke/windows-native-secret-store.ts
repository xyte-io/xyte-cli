...
import { WindowsDpapiSecretStore } from '../../src/secure/secret-store';
...
interface WindowsNativeSecretStoreSmokeOptions {
  cwd?: string;
...
export async function runWindowsNativeSecretStoreSmoke(
  options: WindowsNativeSecretStoreSmokeOptions = {}
): Promise<void> {
  if (process.platform !== 'win32') {
    throw new Error('windows native secret-store smoke must run on Windows.');
  }
  ...
  const tempRoot = await (options.mkdtempFn ?? mkdtemp)(path.join(tmpdir(), 'xyte-cli-windows-native-secret-store-'));
  ...
  const dpapiPath = path.join(configDir, 'secrets.dpapi.v1.json');
  const profilePath = path.join(configDir, 'profile.json');
  const builtCliPath = path.resolve(cwd, 'dist', 'bin', 'xyte-cli.js');
  const nativeStore = new WindowsDpapiSecretStore(dpapiPath, run);
  ...
  try {
    ...
    printStep(logger, 1, stepTotal, 'Checking raw Windows DPAPI CurrentUser round-trip');
    const availability = await nativeStore.checkAvailability();
    if (!availability.available) {
      throw new Error(`Windows DPAPI CurrentUser round-trip failed: ${availability.reason ?? 'unknown reason'}`);
    }
    ...
    if (configPathPayload.secretStoreBackend !== 'dpapi' || configPathPayload.secretStore !== dpapiPath) {
      throw new Error(`CLI did not resolve the native Windows backend: ${JSON.stringify(configPathPayload)}`);
    }
    if (!existsSync(dpapiPath)) {
      throw new Error(`Expected DPAPI store file to exist at ${dpapiPath}`);
    }
    if ((await nativeStore.getSlotSecret('native-smoke', 'xyte-org', 'primary')) !== 'native-smoke-key') {
      throw new Error('Native secret written through setup run did not round-trip from the DPAPI store.');
    }
    ...
    logger.log(`Windows native secret-store smoke passed using ${dpapiPath}.`);
  } finally {
...
if (require.main === module) {
  runWindowsNativeSecretStoreSmoke().catch((error) => {
