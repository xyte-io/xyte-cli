...
const CLIENT_HEADER = 'X-Xyte-Client';
const CLIENT_NAME = 'xyte-cli';
...
export function createXyteClient(options: XyteClientOptions = {}): XyteClient {
  ...
  const callWithMeta = async <T = unknown>(
    ...
    const headers: Record<string, string> = {
      ...
      // Identifies the CLI to the hub, which records it as the originating client
      // of every API call (Settings > API Keys > Call Log).
      [CLIENT_HEADER]: `${CLIENT_NAME}/${getCliVersion()}`
    };
