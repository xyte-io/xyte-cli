...
import { formatErrorText, resolveCliErrorFormat } from '../utils/error-format';
...
runCli().catch((error) => {
  ...
  process.stderr.write(`${redactSensitiveText(formatErrorText(error))}\n`);
  process.exit(1);
});
